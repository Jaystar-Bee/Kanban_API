...
func NewColumnHandler(ctx context.Context, columnCollection *mongo.Collection, taskCollection *mongo.Collection, redisClient *redis.Client) *ColumnHandler {
...
func (handler *ColumnHandler) ListColumnHandler(c *gin.Context) {
	...
	cursor, err := handler.columnCollection.Find(handler.ctx, bson.D{})
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{
			"message": err.Error(),
		})
	}
	err = cursor.All(handler.ctx, &columns)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{
			"message": err.Error(),
		})
